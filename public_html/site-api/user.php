...
/* OLD CODE
function is_404($url) {
    $returned = get_headers('https://opensprites.org/forums/?p=member/' . $url, 1)[0];
    return $returned == 'HTTP/1.1 404 Not Found';
}

function username_of($id) {
    $html = file_get_html('https://opensprites.org/forums/?p=member/' . $id);
...
function avatar_of($id) {
    ...
    if(get_headers('https://opensprites.org/uploads/avatars/' . $id . '.png')[0] == 'HTTP/1.1 404 Not Found') {
        ...
        return 'https://opensprites.org/uploads/avatars/' . $id . '.png';
    }
}
...
error_reporting(0);
$raw = file_get_contents("https://opensprites.org/forums/?p=member/".$userid);
if($raw === FALSE) {
    echo 'FALSE';
} else {
...
///////////////////////// NEWER OLD CODE
$raw = file_get_html("https://opensprites.org/forums/?p=member/" . $userid);
if($raw == FALSE) {
    echo 'FALSE';
    die();
}
...
if($raw->find('img.avatar', 0) !== null) {
    ...
    $avatar = 'https://opensprites.org/' . $raw->find('img.avatar', 0)->src;
} else {
    // they have not uploaded an avatar image
    $avatar = 'https://opensprites.org/assets/images/defaultfile.png';
}
...
$userInfo2['avatar'] = "https://opensprites.org/forums/uploads/avatars/" . $userid . ".png?_=" . time();
